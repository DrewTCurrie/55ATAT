...
function Events() {
    ...
    const generateQuickReport = async () => {
      ...
      try {
        const response = await fetch(`http://192.168.0.54:5000/api/generateReport`, quickReport,).then(
          res => res.json()
        ).then(
           data => {
            axios.get(`http://192.168.0.54:5000/api/download/${data}`,{responseType: 'blob'}
            ).then((downloadRes) => {
